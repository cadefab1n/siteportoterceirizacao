...
const SafeImage = ({ 
  ...
  skipDefaultImageFallback = false
}: { 
  src: string; 
  alt: string; 
  className?: string; 
  ...
  skipDefaultImageFallback?: boolean;
}) => {
    ...
    <div className="relative w-full h-full bg-[#14283D] flex items-center justify-center overflow-hidden">
      {!hasError ? (
        <img 
          ...
          referrerPolicy="no-referrer"
          onError={() => {
            if (skipDefaultImageFallback) {
              setHasError(true);
            } else if (imgSrc !== "/@portoterceirização.png") {
